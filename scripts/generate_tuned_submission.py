"""Generate submissions using tuned hyperparameters.

Loads tuned params from artifacts/tuned_params.json, re-runs ensemble CV
with tuned HPs to get proper OOF predictions, re-optimizes ensemble weights,
then trains final models and generates submissions.
"""
import json
import sys
import time
...
def main():
    ...
    # Load tuned params
    with open('artifacts/tuned_params.json') as f:
        tuned = json.load(f)
    ...
    for gender, df in [('M', df_m), ('W', df_w)]:
        ...
        eoa_result = tune_ensemble_weights_eoa(
            ...
            log_dir=f'runs/ensemble_weights_tuned_{gender}_eoa',
        )
        ...
        ax_result = tune_ensemble_weights_ax(
            oof_dict, y_true,
            n_trials=100,
            log_dir=f'runs/ensemble_weights_tuned_{gender}_ax',
        )
    ...
    print("\nSaved updated artifacts/tuned_params.json")
    ...
    generate_submission(
        ...
        stage=1,
        output_path='submissions/ensemble_tuned_v1.csv',
    )
    print("Validating ensemble_tuned_v1.csv:")
    validate_submission('submissions/ensemble_tuned_v1.csv', expected_rows=519144)

    print("\nGenerating Stage 2 submission...")
    generate_submission(
        predict_fn=predict_fn,
        stage=2,
        output_path='submissions/ensemble_tuned_2026.csv',
    )
    print("Validating ensemble_tuned_2026.csv:")
    validate_submission('submissions/ensemble_tuned_2026.csv')
