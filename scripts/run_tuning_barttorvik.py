"""Hyperparameter tuning with Barttorvik features (47-feature set).

Same pipeline as run_tuning.py but writes to *_barttorvik_v2* output paths
so original tuning results are never overwritten.
"""
import json
import sys
sys.path.insert(0, '.')
...
def tune_all_models(df, feature_cols, gender, eoa_epochs=100, ax_trials=50):
    ...
    Checkpoints after each model completes.
    """
    ...
    for model_name in models_to_tune:
        ...
        eoa_result = tune_model_eoa(
            ...
            log_dir=f'runs/{model_name}_{gender}_bt_eoa',
        )
        ...
        ax_result = tune_model_ax(
            model_name, df, feature_cols, gender,
            n_trials=ax_trials,
            log_dir=f'runs/{model_name}_{gender}_bt_ax',
        )
        ...
        print(f"  [checkpoint] {model_name} ({gender}) done — best: {best_brier:.4f}")
        checkpoint_data = {f'{gender}_models_partial': results}
...
def tune_ensemble_weights_both(oof_preds_dict, y_true, gender):
    ...
    eoa_result = tune_ensemble_weights_eoa(
        ...
        log_dir=f'runs/ensemble_weights_{gender}_bt_eoa',
    )
    ...
    ax_result = tune_ensemble_weights_ax(
        oof_preds_dict, y_true,
        n_trials=100,
        log_dir=f'runs/ensemble_weights_{gender}_bt_ax',
    )
...
CHECKPOINT_PATH = 'artifacts/tuning_barttorvik_v2_checkpoint.json'
...
def main():
    ...
    # --- Save final results ---
    with open('artifacts/tuning_results_barttorvik_v2.json', 'w') as f:
        json.dump(jsonify(all_results), f, indent=2)
    print("\nSaved artifacts/tuning_results_barttorvik_v2.json")
    ...
    with open('artifacts/tuned_params_barttorvik_v2.json', 'w') as f:
        json.dump(jsonify(tuned_params), f, indent=2)
    print("Saved artifacts/tuned_params_barttorvik_v2.json")
